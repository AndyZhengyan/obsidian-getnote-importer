export const translations: Record<string, Record<string, string>> = {
  zh: {
    ...
    'settings.credentials.webTip': '网页模式使用网页版登录后的 Authorization Bearer Token，可选填写 xi-csrf-token；Token 可能约 8 天后失效。',
    'settings.authMode.openapi': 'OpenAPI 模式（PRO）',
    'settings.authMode.web': '网页模式（手动 Token）',
    'settings.webToken.placeholder': 'Authorization Bearer Token（可含 Bearer 前缀）',
    'settings.webCsrf.placeholder': 'xi-csrf-token（可选）',
    'settings.syncPicker.button': '按笔记同步',
    'settings.autoSync': '自动同步',
    'settings.manualSync': '手动同步',
    'settings.credentials.label': '凭证设置',
    'settings.onboarding': '👋 欢迎使用！请先选择认证方式并填写凭证，然后点击同步。OpenAPI 适合 PRO 用户，网页模式可手动粘贴网页版 Token。',
    ...
    'notice.quotaExceededStop': '[GetNote] API 配额已用完，定时同步已暂停。请明天再试。',
  ...
  en: {
    ...
    'settings.credentials.webTip': 'Web mode uses the Authorization Bearer token from an active GetNote web session. xi-csrf-token is optional; the token may expire after about 8 days.',
    'settings.authMode.openapi': 'OpenAPI mode (PRO)',
    'settings.authMode.web': 'Web mode (manual token)',
    'settings.webToken.placeholder': 'Authorization Bearer token (Bearer prefix accepted)',
    'settings.webCsrf.placeholder': 'xi-csrf-token (optional)',
    ...
    'settings.onboarding': '👋 Welcome! Choose an authentication mode and enter credentials first. OpenAPI is for PRO users; Web mode accepts a manually copied web token.',
    ...
    'notice.quotaExceededStop': '[GetNote] API quota exhausted. Scheduled sync has been paused. Please try again tomorrow.',
