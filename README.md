...
- **🚦 自动同步不再撞上游限流**：修复了一个让双向同步每次都把上游读接口打满的 bug——现在只在本地文件有改动时才重新拉取详情，自动同步不再因上游速率限制而失败。中间被中断的同步也会从断点续跑，不会从头再来。
- **📝 划线笔记同步有内容了**：划线笔记从云端同步过来不再变成空正文加 `(无标题)` 文件名，会保留原始划线内容并归到正确标题。
- **📄 文字笔记不再丢失正文**：列表接口偶发返回空 content 的文字笔记，现在会走详情接口补全，不再以空 body + `(无标题)` 文件名落库（#309）。

这是一次稳定性修复，建议所有开启自动同步的用户升级。
