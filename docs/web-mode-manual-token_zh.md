...
Web 模式适合无法使用 Get笔记 OpenAPI 的用户。它复用浏览器里已经登录的 Get笔记网页版会话，所以只需要浏览器请求里的 `Authorization` header，不需要 `Client ID`。
...
- 你可以在 `https://www.biji.com/note` 正常登录 Get笔记网页版。
...
- 如果 `测试连接` 返回 `401`、`403` 或 `Web Token 已过期`，刷新 Get笔记网页版并重新复制 `Authorization` header。
