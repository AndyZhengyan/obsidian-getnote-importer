...
- **🔢 大编号不再失真**：得到大脑笔记的长数字 ID 按字符串保存和比较，避免精度丢失引发重复关联与同步错误。
- **🔄 同步状态更可信**：未解决的差异会明确显示；临时错误可在重试成功后清除，部分成功的同步也保留恢复依据。
- **🛡️ 旧笔记安全对齐**：正文与图片对象相同的旧笔记可按远端建立基线；归档或仅保留本地的副本退出同步，搜索也不会误认作当前笔记。
- **⚙️ 双向同步提示更清楚**：设置项解释临时鉴权限制，并在禁用状态保留悬停说明。
