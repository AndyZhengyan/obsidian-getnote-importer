    ...
    vi.spyOn(obsidian, 'requestUrl').mockImplementation(() => {
      ...
      if (page === 1) {
        return Promise.resolve({
          ...
          text: JSON.stringify({
            data: {
              notes: [
                makeNote({ note_id: 'early', updated_at: '2026-05-10T10:00:00+08:00' }),
              ],
              has_more: true,
              next_cursor: 'page2',
            },
      ...
      return Promise.resolve({
        ...
        text: JSON.stringify({
          data: {
            notes: [
              makeNote({ note_id: 'newer', updated_at: '2026-05-12T10:00:00+08:00' }),
            ],
            has_more: false,
            next_cursor: '',
          },
...
describe('SyncEngine — seenNoteIds cross-page dedup', () => {
  it('deduplicates notes that appear on multiple pages', async () => {
    ...
    // 3 unique notes total
    expect(result.total).toBe(3);
    expect(result.created).toBe(3);
    // note_B should NOT be double-counted
    const uniqueNoteIds = new Set(result.items!.map(i => i.noteId));
    expect(uniqueNoteIds.size).toBe(3);
    // Verify no duplicate noteId appears in items
      ...
      if (page === 1) {
        return Promise.resolve({
          ...
          text: JSON.stringify({
            data: {
              notes: [
                makeNote({ note_id: 'only_A', title: 'Only A' }),
              ],
              has_more: true,
              next_cursor: 'page2',
            },
...
describe('SyncEngine — lastSyncEndTimestamp boundary re-check', () => {
  it('boundary note at lastSyncEndTimestamp is now excluded by layer-1 filter (>)', async () => {
    vi.spyOn(obsidian, 'requestUrl').mockResolvedValue({
      status: 200,
      headers: { 'content-type': 'application/json' },
      text: JSON.stringify({
    ...
    // Boundary note excluded by Layer 1 (> filter) — no longer re-processed
    expect(result.total).toBe(0);
    expect(result.skipped).toBe(0);
    expect(result.lastNoteTimestamp).toBeUndefined();
  });
  ...
  it('newer notes (> boundary) pass the > filter and advance lastSyncEndTimestamp', async () => {
        ...
        data: {
          notes: [
            makeNote({
              note_id: 'new_note',
              title: '新笔记',
              updated_at: '2026-05-11T12:00:00+08:00', // > startDate → kept
            }),
            makeNote({
              note_id: 'boundary_note',
              title: '边界笔记',
              updated_at: '2026-05-10T12:00:00+08:00', // == startDate → excluded
            }),
          ],
          has_more: false,
          next_cursor: '',
    ...
    // Only new_note (> boundary) passes Layer 1 filter
    expect(result.total).toBe(1);
    expect(result.created).toBe(1);
    expect(result.skipped).toBe(0);
    // lastNoteTimestamp advances past the boundary
    expect(result.lastNoteTimestamp).toBe('2026-05-11T12:00:00+08:00');
  });
});
