...
- **🔄 本地修改可自动回传**：自动同步可选择“本地修改自动上传”，同步目录中已关联的文字笔记会安全回传；两端同时修改时保留双方内容，交给手动同步处理冲突。
- **📝 本地草稿一次上传并归档**：同步目录中新建的 Markdown 文字笔记可自动创建到得到大脑，写回 `uid` 后移动到规范路径；目标文件已存在时保留原文件并明确报错，不会覆盖。
- **📚 知识库归属不会丢**：在 `知识库/<名称>/` 中新建的草稿会加入对应的可写云端知识库；无法确认归属或知识库只读时停止上传，避免产生无归属笔记。
- **🎛️ 自动同步设置更直接**：下载与上传合并到同一个自动同步区域，用一个开关决定“仅下载”或“下载并上传”；空结果也会给出明确反馈。
- **🧪 工具链更新**：升级测试与构建依赖，包括 Vitest 5，并继续验证 OpenAPI 和 Web API 的鉴权行为。

这是一次双向同步能力更新。自动上传默认关闭且需要 OpenAPI；升级后可在“自动同步”中按需开启“本地修改自动上传”。
...
开启自动同步后，插件会按设定间隔从得到大脑同步到 Obsidian，也可设置启动时同步。启用“本地修改自动上传”后，同一轮还会处理同步目录（含子目录）中新建或修改的文字笔记。
...
- 不会自动覆盖得到大脑已有笔记。
- 同步目录中已关联文字笔记的修改，可由“本地修改自动上传”处理。
- 上传标签会去重，并限制数量。
...
- ⬆️ 手动上传只发送你明确选择的 Markdown；启用“本地修改自动上传”后，会发送同步目录（含子目录）中符合条件的新建或已修改文字笔记。
- 📎 附件只从得到大脑接口返回的地址下载。
